...
public class DiffService
{
        ...
        int version)
    {
        ...
        foreach (var rowId in allRowIds)
        {
            ...
            else if (hasOldRow && hasNewRow)
            {
                ...
                // This provides complete audit trail showing how calculated values evolved
                foreach (var colDef in columnDefs)
                {
                    oldRow!.Cells.TryGetValue(colDef.Field, out var oldValue);
                    newRow!.Cells.TryGetValue(colDef.Field, out var newValue);

                    _logger.LogInformation("Comparing {RowId}, {ColumnField}, Old: {OldValue} ({OldType}), New: {NewValue} ({NewType})", 
                        rowId, colDef.Field, oldValue, oldValue?.GetType().Name, newValue, newValue?.GetType().Name);

                    if (!AreValuesEqual(oldValue, newValue))
                    {
                        _logger.LogInformation("Found diff: {RowId}, {ColumnField}, Old: {OldValue} ({OldType}), New: {NewValue} ({NewType})", 
                            rowId, colDef.Field, oldValue, oldValue?.GetType().Name, newValue, newValue?.GetType().Name);
                        auditEntries.Add(new AuditLogEntryDto
                        {
                            Version = version,  
    ...
    private bool AreValuesEqual(object? value1, object? value2)
    {
        ...
        var actualValue1 = Utilities.JsonHelper.ExtractValue(value1);
        var actualValue2 = Utilities.JsonHelper.ExtractValue(value2);
