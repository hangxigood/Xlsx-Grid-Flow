...
public class FormulaService
{
        ...
        try
        {
            ...
            foreach (var row in rowData)
            {
                foreach (var colDef in columnDefs)
                {
                    ...
                    var actualValue = Utilities.JsonHelper.ExtractValue(cellValue);
            ...
            foreach (var row in rowData)
            {
                ...
                foreach (var colDef in columnDefs)
                {
                    ...
                    if (!string.IsNullOrEmpty(cell.Formula))
                    {
                        ...
                        // This allows the diff service to detect when results change
    ...
    /// <summary>
    /// Validates that formula cells in newData match backend calculations
        ...
        List<GridRowDto> rowData)
    {
        try
        {
            var recalculated = RecalculateFormulas(columnDefs, rowData);
            
            // Compare formula results
            foreach (var (original, calculated) in rowData.Zip(recalculated))
            {
                foreach (var colDef in columnDefs.Where(c => c.DataType == DataType.Formula))
                {
                    var originalValue = original.Cells.GetValueOrDefault(colDef.Field);
                    var calculatedValue = calculated.Cells.GetValueOrDefault(colDef.Field);

                    // Both should be formula strings
                    if (originalValue?.ToString() != calculatedValue?.ToString())
                    {
                        _logger.LogWarning(
                            "Formula mismatch in {Field} row {RowId}: expected {Expected}, got {Actual}",
                            colDef.Field, original.RowId, calculatedValue, originalValue);
                        return false;
                    }
                }
            }
        ...
        {
            _logger.LogError(ex, "Formula validation failed");
            return false;
        }
    }
